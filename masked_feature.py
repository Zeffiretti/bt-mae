...
class MaskMViT(MViT):
    def __init__(self, cfg):
        ...
        if self.head_type[0] == "separate":
            if not cfg.MASK.PRED_HOG:
                pred_t_sz = 1 if self.cfg.MASK.TIME_STRIDE_LOSS else self.patch_stride[0]
                num_classes = [pred_t_sz * (self.feat_stride[depth][-1] ** 2) * 3 for depth in self.pretrain_depth]
                self.pred_head = head_helper.MSSeparateHead(self.blocks, cfg, num_classes, feat_sz)
            else:
                ...
                self.ncells = [(self.feat_stride[depth][-1] // self.cell_sz) ** 2 for depth in self.pretrain_depth]
                pred_hog_classes = [self.nbins * ncell for ncell in self.ncells]
                pred_hog_classes = [pred_hog_class * 3 for pred_hog_class in pred_hog_classes]  # 3 color channels
                self.pred_head = head_helper.MSSeparateHead(self.blocks, cfg, pred_hog_classes, feat_sz)
                self.hog_loss = "mse"
        else:
        ...
        if cfg.MASK.MAE_ON:
            ...
            if self.use_abs_pos:
                if self.sep_pos_embed_decoder:
                    ...
                    self.dec_pos_embed_temporal = nn.Parameter(torch.zeros(1, self.patch_dims[0], decoder_embed_dim))
                    if self.cls_embed_on:
                        self.dec_pos_embed_class = nn.Parameter(torch.zeros(1, 1, decoder_embed_dim))
                else:
        ...
        self.mask_token = nn.Parameter(torch.zeros(1, 1, decoder_embed_dim if cfg.MASK.MAE_ON else embed_dim))
    ...
    def _get_pixel_label_3d(self, input_frames, output_masks, time_stride_loss=True, norm=True):
        if time_stride_loss:
    ...
    def _get_hog_label_3d(self, input_frames, output_masks):
        input_frames = input_frames[:, :, :: self.cfg.MVIT.PATCH_STRIDE[0], :, :]  # B C T H W
        ...
        for depth, output_mask in zip(self.pretrain_depth, output_masks):
            ...
            for hog in self.hogs:
                ...
                unfold_size = tmp_hog.shape[-1] // feat_size
                tmp_hog = (
                    tmp_hog.permute(0, 2, 3, 1).unfold(1, unfold_size, unfold_size).unfold(2, unfold_size, unfold_size)
                )  # BT h w C wh ww
                tmp_hog = tmp_hog.flatten(3).view(B, T, feat_size, feat_size, -1)  # B T h w C (3 nbins h w)
    ...
    def _mae_random_masking(self, x, mask_ratio, mask_in=None):
        ...
        if mask_in is None:
            if self.cfg.AUG.MASK_TUBE:
                noise = (
                    torch.rand(N, 1, self.H * self.W, device=x.device).repeat([1, self.T, 1]).reshape(N, L)
                )  # noise in [0, 1]
            else:
    ...
    def _mae_forward_encoder(self, x, mask_ratio, mask=None):
        ...
        if self.use_abs_pos and not self.use_fixed_sincos_pos:
            if self.sep_pos_embed:
                ...
                if self.cls_embed_on:
                    pos_embed_sampled = torch.cat([pos_embed[:, :s, :], pos_embed_sampled], 1)
                x += pos_embed_sampled
    ...
    def _mae_forward_decoder(self, x, ids_restore, mask, thw):
        ...
        mask_tokens = self.mask_token.repeat(B, T * H * W + s - x.shape[1], 1)  # + s: no cls token
        ...
        if self.sep_pos_embed_decoder:
            pos_embed = self.dec_pos_embed_spatial.repeat(1, self.patch_dims[0], 1) + torch.repeat_interleave(
    ...
    def _mae_forward(self, imgs, mask_ratio=0.75, mask=None):
        latent, mask, ids_restore, thw = self._mae_forward_encoder(imgs, mask_ratio, mask)
    ...
    def _mae_visualize(self, imgs, pred, mask):
        ...
        for i in range(comparison.shape[0]):
            misc.plot_input_normed(
                ...
                + "/vis_mask/vid/{}vis_video_in_mask_out_mr{}".format(pfx[pfx.rfind("/") + 1 : -5], mr),
                make_grids=True,
                output_video=True,
            )
        return pred[0]
    ...
    def _maskfeat_forward(self, x, mask, return_all=False):
        ...
        if self.cfg.MASK.MAE_RND_MASK:
            _, mask, ids_restore, ids_keep = self._mae_random_masking(x_embed, self.cfg.AUG.MASK_RATIO, None)
        ...
        if self.pred_pixel_wt:
            if self.use_2d_patch:
                labels += self._get_pixel_label_2d(x.detach(), output_masks, norm=self.cfg.MASK.NORM_PRED_PIXEL)
            else:
                labels += self._get_pixel_label_3d(x.detach(), output_masks, norm=self.cfg.MASK.NORM_PRED_PIXEL)
        if self.pred_hog_wt:
            if self.use_2d_patch:
